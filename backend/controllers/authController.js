...
export const register = async (req, res) => {
  try {
    const { userData, token, emailVerificationToken } = await registerUser(
      req.body
    );
    ...
    try {
      ...
      logger.error(
        `Failed to send welcome email to ${userData.email}: ${emailError.message}`
      );
    }
    ...
    try {
      ...
      logger.error(
        `Failed to send verification email to ${userData.email}: ${emailError.message}`
      );
    }
