...
export const getDailyRecommendation = async (req, res) => {
  try {
    ...
    if (!todayRecommendation) {
      logger.info(`No pre-generated recommendation found for user ${userId}, generating on-demand`);
      ...
      logger.info(`Successfully generated on-demand recommendation for user ${userId}`);
      return res.json({
