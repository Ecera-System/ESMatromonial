...
const ViewProfile = ({ profileData: propProfileData, onBackToCreate }) => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Use profile data from props or location state
  const profileData = propProfileData || location.state?.profile || {};
  ...
  const handleBackToCreate = () => {
    // Always navigate back to the home route (CreateProfile)
    navigate('/');
  };

  const renderPhotos = () => {
    // Handle both File objects (from form) and URL strings (from database)
    ...
    return (
      <div className="grid grid-cols-1 gap-4">
        {profileData.photos.map((photo, index) => {
          // Check if photo is a File object or a database photo object
          const photoUrl = photo.url || (photo instanceof File ? URL.createObjectURL(photo) : photo);
