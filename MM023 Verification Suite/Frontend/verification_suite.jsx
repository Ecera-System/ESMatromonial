        ...
        <div className="bg-white/20 backdrop-blur-lg shadow-lg rounded-3xl p-10 w-full max-w-xl border border-white/30 transition-all duration-500 ease-in-out animate-fade-in">
          ...
          {step === "aadhar-upload" && (
            <div className="space-y-6 animate-fade-in">
              ...
              <button
                onClick={async () => {
                  if (!aadharFile) return alert("Please upload a file");

                  const formData = new FormData();
                  formData.append("aadhaar", aadharFile);

                  try {
                    const res = await fetch("http://localhost:5000/api/verify-aadhaar", {
                      method: "POST",
                      body: formData,
                    });
                    ...
                    if (data.verified) {
                      ...
                      setMockAadharNumber(data.uid);
                      setStep("aadhar-mobile");
                    } else {
                      setToast("Aadhaar verification failed: " + (data.error || "Invalid QR"));
                    }
                  } catch (err) {
                    setToast("Server error during Aadhaar verification");
                  }
          ...
          {step === "aadhar-otp" && (
            <div className="space-y-6 animate-fade-in">
              ...
              <button
                onClick={() => {
                  setToast("Aadhaar OTP verified successfully");
                  setTimeout(() => setToast(""), 3000);
                }}
