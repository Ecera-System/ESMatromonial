...
const AdminDashboard = () => {
  ...
  const statsCards = [
    { title: 'Total Users', value: '12,456', icon: Users, color: 'bg-blue-500', change: '+12%' },
    { title: 'Female Users', value: '200', icon: UserCheck2, color: 'bg-green-500', change: '+8%' },
    { title: 'Total Premium Users', value: '3,567', icon: Crown, color: 'bg-purple-500', change: '+25%' },
    { title: 'Male Users', value: '100', icon: User, color: 'bg-red-500', change: '-5%' },
  ];

  // Mock data for recently registered users
  const recentUsers = [
    { id: 1, name: 'John Doe', email: 'john@example.com', avatar: '/api/placeholder/40/40', joinedAt: '2024-01-15' },
    { id: 2, name: 'Jane Smith', email: 'jane@example.com', avatar: '/api/placeholder/40/40', joinedAt: '2024-01-14' },
    { id: 3, name: 'Mike Johnson', email: 'mike@example.com', avatar: '/api/placeholder/40/40', joinedAt: '2024-01-13' },
    { id: 4, name: 'Sarah Wilson', email: 'sarah@example.com', avatar: '/api/placeholder/40/40', joinedAt: '2024-01-12' },
    { id: 5, name: 'David Brown', email: 'david@example.com', avatar: '/api/placeholder/40/40', joinedAt: '2024-01-11' },
  ];
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`hidden lg:block ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b px-6 py-4`}>
            <div className="flex items-center justify-between">
              <div>
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} capitalize`}>{activeTab}</h2>
          ...
          <div className="p-4 lg:p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  {statsCards.map((card, index) => (
                    <div key={index} className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-6 border hover:shadow-md transition-shadow`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} font-medium`}>{card.title}</p>
                          <p className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mt-1`}>{card.value}</p>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl border`}>
                    <div className={`p-6 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} border-b`}>
                      <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Recently Registered Users</h3>
                    </div>
                    <div className="p-6">
                      <div className="space-y-4">
                        {recentUsers.slice(0, 5).map((user) => (
                          <div key={user.id} className={`flex items-center space-x-4 p-4 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg`}>
                            ...
                            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{new Date(user.joinedAt).toLocaleDateString()}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl border`}>
                    <div className={`p-6 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} border-b`}>
                      <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Search Users</h3>
                    </div>
                    <div className="p-6">
                      <div className="mb-4">
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={`w-full pl-10 pr-4 py-2 border ${isDarkMode ? 'border-gray-600 bg-gray-700 text-white' : 'border-gray-300 bg-white'} rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent`}
                          />
                        </div>
                      </div>
                      <div className="space-y-4 max-h-64 overflow-y-auto">
                        ...
                        {filteredUsers.length === 0 && searchTerm && (
                          <div className="text-center py-4">
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>No users found matching "{searchTerm}"</p>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {/* Other tabs content */}
            {!['overview', 'users', 'feedback', 'inactive-users'].includes(activeTab) && (
